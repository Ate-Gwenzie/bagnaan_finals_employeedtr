...
if (isset($_GET['delete_id'])) {
    ...
    if ($deleteId > 0) {
        $notifications_check = $notificationManager->getNotifications('admin', $adid);
        $is_owner = false;
        ...
        if ($is_owner && $notificationManager->deleteNotification($deleteId)) {
            header("Location: viewNotifications.php?status=deleted");
            exit();
        } else {
            header("Location: viewNotifications.php?error=delete_failed");
            exit();
        }
    }
}
...
$notifications = $notificationManager->getNotifications('admin', $adid);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="app-container notification-view">
    ...
    <?php if (empty($notifications)): ?>
        <div class="alert alert-info">You have no administrative notifications.</div>
    <?php else: ?>
